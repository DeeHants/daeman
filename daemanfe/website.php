...
?>
  ...
  <form action="website.php" method="POST">
   ...
   <table>
    <tr><td>Host name</td><td><input name="host" value="<?php print htmlspecialchars($host[0]['Host']); ?>"></td></tr>
...
?>
  ...
  <form action="website.php" method="POST">
   ...
   <input name="websiteid" type="hidden" value="<?php print htmlspecialchars($wdetails['ID']); ?>">
   <table>
    <tr><td>Host name</td><td><input name="host"></td></tr>
...
print "  <h2>Tools</h2>\n";
$serveraddress = execute("SELECT FullName FROM Websites Left JOIN Servers ON Servers.ID=Websites.ServerID WHERE Websites.ID='" . mysql_escape_string($wdetails['ID']) . "';");
$serveraddress = $serveraddress[0]['FullName'];
...
if ($wdetails['Logging']) {
  ...
  print "  <p>You can also access the log fires directly from the HTTP server at <a href=\"ftp://" . urlencode($details['Name']) . "@" . $serveraddress . "/websites/" . urlencode($wdetails['Name']) . "_log\">ftp://" . urlencode($details['Name']) . "@" . $serveraddress . "/websites/" . urlencode($wdetails['Name']) . "_log</a>.\n";
}
print "  <p>You can upload your website using FTP. Here is a link to your website directory upload area. <a href=\"ftp://" . urlencode($details['Name']) . "@" . $serveraddress . "/websites/" . urlencode($wdetails['Name']) . "\">Enter upload area</a>.\n";
