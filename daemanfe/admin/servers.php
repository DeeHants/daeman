...
if (isset($action)){
  if ($action == "addserver"){
    if (execute("INSERT INTO Servers (Name, Address, DNS, Mail, HTTP, DB) VALUES ('" . mysql_escape_string($name) . "', '" . mysql_escape_string($address) . "', '" . mysql_escape_string($dns) . "', '" . mysql_escape_string($mail) . "', '" . mysql_escape_string($http) . "', '" . mysql_escape_string($db) . "');")){
      print "  <p class=status>Server added successfully.</p>\n";
    }else{
      print "  <p class=error>Error adding server.</p>\n";
    }
  }elseif ($action == "updateserver"){ 
    if (execute("UPDATE Servers SET Name='" . mysql_escape_string($name) . "', Address='" . mysql_escape_string($address) . "', DNS='" . mysql_escape_string($dns) . "', Mail='" . mysql_escape_string($mail) . "', HTTP='" . mysql_escape_string($http) . "', DB='" . mysql_escape_string($db) . "' WHERE ID='" . mysql_escape_string($serverid) . "';")){
...
$servers = execute("SELECT ID, Name, Address, DNS, Mail, HTTP, DB FROM Servers ORDER BY Name;");
if ($servers){
?>
  <table backcolour=red>
   <tr><th>Actions</th><th>Name</th><th>Address</th><th>DNS</th><th>Mail</th><th>HTTP</th><th>DB</th></tr>
<?php
  for($row = 0; $row < count($servers); $row++){
    print "   <tr><td><div class=action><a href=\"servers.php?action=editserver&amp;serverid=" . $servers[$row]['ID'] . "#serverform\">edit</a> <a href=\"servers.php?action=deleteserver&amp;serverid=" . $servers[$row]['ID']. "\">delete</a></div></td><td>" . htmlspecialchars($servers[$row]['Name']) . "</td><td>" . htmlspecialchars($servers[$row]['Address']) . "</td><td>" . iif($servers[$row]['DNS'], "Yes", "No") . "</td><td>" . iif($servers[$row]['Mail'], "Yes", "No") . "</td><td>" . iif($servers[$row]['HTTP'], "Yes", "No") . "</td><td>" . iif($servers[$row]['DB'], "Yes", "No") . "</td></tr>\n";
  }
?>
  </table>
<?php
...
if ($action == "editserver"){
  $server = execute("SELECT Name, Address, DNS, Mail, HTTP, DB FROM Servers WHERE ID='" . mysql_escape_string($serverid) . "';");
?>
  <a name=serverform>
