...
if (isset($action)){
  if ($action == "addserver"){
    if (execute("INSERT INTO Servers (Name, FullName, Address, Live, DNS, Mail, HTTP, DB, Shell) VALUES ('" . mysql_escape_string($name) . "', '" . mysql_escape_string($fullname) . "', '" . mysql_escape_string($address) . "', '" . mysql_escape_string($live) . "', '" . mysql_escape_string($dns) . "', '" . mysql_escape_string($mail) . "', '" . mysql_escape_string($http) . "', '" . mysql_escape_string($db) . "', '" . mysql_escape_string($shell) . "');")){
      print "  <p class=status>Server added successfully.</p>\n";
    }else{
      print "  <p class=error>Error adding server.</p>\n";
    }
  }elseif ($action == "updateserver"){ 
    if (execute("UPDATE Servers SET Name='" . mysql_escape_string($name) . "', FullName='" . mysql_escape_string($fullname) . "', Address='" . mysql_escape_string($address) . "', Live='" . mysql_escape_string($live) ."', DNS='" . mysql_escape_string($dns) . "', Mail='" . mysql_escape_string($mail) . "', HTTP='" . mysql_escape_string($http) . "', DB='" . mysql_escape_string($db) . "', Shell='" . mysql_escape_string($shell) ."' WHERE ID='" . mysql_escape_string($serverid) . "';")){
...
$servers = execute("SELECT ID, Name, FullName, Address, Live, DNS, Mail, HTTP, DB, Shell FROM Servers ORDER BY Name;");
if ($servers){
?>
  <table backcolour=red>
   <tr><th>Actions</th><th>Name</th><th>Full name</th><th>Address</th><th>Live</th><th>DNS</th><th>Mail</th><th>HTTP</th><th>DB</th><th>Shell</th></tr>
<?php
...
if ($action == "editserver"){
  $server = execute("SELECT Name, FullName, Address, Live, DNS, Mail, HTTP, DB, Shell FROM Servers WHERE ID='" . mysql_escape_string($serverid) . "';");
?>
  <a name=serverform>
