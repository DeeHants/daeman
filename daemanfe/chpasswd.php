...
print_header("Change password: " . htmlspecialchars($details['Name']));
...
if (isset($action)){
  if ($action == "setpassword"){
    if ($newpassword != $newpasswordrepeat){
      ...
      if (execute("UPDATE Users SET Password = Encrypt('" . mysql_escape_string($newpassword) . "', Concat('$1$', Round(Rand()*100000))), DBPassword = Password('" . mysql_escape_string($newpassword) . "') WHERE ID='" . mysql_escape_string($userid) . "';")){
